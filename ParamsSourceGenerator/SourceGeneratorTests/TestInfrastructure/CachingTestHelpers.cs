...
public static class CachingTestHelpers
{
    public static async Task AssertOutputsMatch(GeneratorDriverRunResult runResult, ICollection<CSharpFile> files)
    {
        ...
        await Assert.That(actualFileNames).IsEquivalentTo(expectedFileNames);

        foreach (var file in files)
        {
            var tree = runResult.GeneratedTrees.First(e => Path.GetFileName(e.FilePath) == file.Name);
            await Assert.That(file.Content).IsEqualTo(tree.ToString());
        }
    }

    public static async Task AssertAllOutputs(GeneratorDriverRunResult runResult, IncrementalStepRunReason reason)
    {
        ...
        {
            await Assert.That(output.Reason).IsEqualTo(reason);
        }
    }

    public static async Task AssertOutput(
        ...
        IncrementalStepRunReason reason)
    {
        var concreteOutputs = runResult.Results[0].TrackedOutputSteps
            ...
            .ToList();
        await Assert.That(concreteOutputs).HasSingleItem();
        ...
        string[] trackingNames)
    {
        ...
        await Assert.That(trackedSteps1.Count).IsEqualTo(trackedSteps2.Count);
        foreach (var key in trackedSteps2.Keys)
        {
            await Assert.That(trackedSteps1).ContainsKey(key);
        }
        ...
        string stepName)
    {
        ...
        for (var i = 0; i < runSteps1.Length; i++)
        {
            ...
            await Assert.That(outputs1).IsEquivalentTo(outputs2);
            ...
            {
                await Assert
                    .That(output.Reason is IncrementalStepRunReason.Cached or IncrementalStepRunReason.Unchanged)
                    .IsTrue()
                    .Because(
